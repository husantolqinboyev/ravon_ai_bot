          ...
          <CardContent>
            <div className="space-y-3">
              {[1, 2, 3].map((num) => (
                <div
                  key={num}
                  className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${referralCount >= num ? 'bg-primary/5 border-primary/20' : 'bg-muted/30 border-border'
                    }`}
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${referralCount >= num ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                    }`}>
